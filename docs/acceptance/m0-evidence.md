...
| M0.10 | Web dashboard shell, error pages, navigation, CSP, health/integration/pending-count display | `frontend/` builds the Vue SPA as a separate service; `src/careerops/web/api_auth.py` exposes the versioned auth API; the frontend forwards same-origin `/api/*` requests to the backend; the backend does not serve frontend HTML. | Accepted for M0 local engineering evidence. |
| M0.11 | Local Compose topology: API, frontend, workflow-worker, PostgreSQL, Redis, Temporal, Temporal UI; no Browser Worker | `docker-compose.yml` defines separate `frontend` and `api` services plus `postgres`, `redis`, `temporal`, `temporal-ui`, `migration`, `workflow-worker`, and `smart-intake-retention`; no Browser Worker exists; frontend, API, and Temporal UI are loopback-published; PostgreSQL/Redis/Temporal remain internal. All external images are digest-pinned and audited. | Accepted for M0 local Compose evidence. |
| M0.12 | CI for lint, format check, typecheck, unit tests, migration/integration, security and secret scans | `.github/workflows/ci.yml` pins GitHub actions by commit SHA; quality runs `make verify`, Bandit, and secret scan; PostgreSQL job bootstraps roles and runs `make verify-db`; Compose job runs config validation, `docker compose up --build --wait`, API health, frontend shell, and same-origin `/api` forwarding probes. | Local evidence accepted; live CI run evidence is outside this working-tree pass. |
