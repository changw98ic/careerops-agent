...
All tables have candidate-scoped indexes and foreign keys. The API role has only the required row-scoped read/write privileges; append-only stage events cannot be updated/deleted by the API role. Retention uses a controlled security-definer purge function and a retention role, not ad hoc deletes. The migration backfills canonical lifecycle fields using the mapping above, preserves the old state, and is tested against an old application/new schema and new application/old rows. Production rollback is forward-only: stop new workflow scheduling, dual-read legacy fields, reconcile active runs, and keep historical events/audit data. The Alembic downgrade is a guarded disposable-database teardown: it fails closed when any Agent row exists, and only removes 0030-owned objects from an empty schema before restoring the 0025 foreign key and grants.
