...
- [x] 2.5 Verify the mail/outbox schedules and paused crawl schedules survive a worker restart without duplicate side effects, using existing `run_identity` and send idempotency. _(已实现并 live 验证：`tests/unit/test_loop_bootstrap_idempotent.py` 双次 bootstrap 断言 create-or-reconcile 收敛、节奏不漂移；2026-07-31 真实 102 来源验收后重启 production worker，复查 `sources=102|present=102|active=102|hourly=102`，Schedule 无丢失、无重复。)_
...
- [x] 8.8 Run the production crawl-to-inbox path against a broad set of real, independent job websites without a three-posting test cap; retain per-source run/attempt receipts and aggregate counts for discovered postings, canonical jobs, persisted matches, and inbox decisions. The acceptance run must demonstrate actual multi-site scanning at scale; simulated 100-source capacity tests and a one-source smoke test do not satisfy this requirement. _(2026-07-31 production 验收通过：102 个真实独立 ATS 网站逐来源运行，`max_postings_per_source=5000`；102 条 durable attempt 中 `postings_found=97`、`verified_empty=5`；真实 `postings=14934`、`canonical_jobs=14666`、`matched_jobs=14666`、`inbox_jobs=14666`、失败岗位计数 0。102 个一小时 Schedule 全部启用，并在 worker 重启后保持 `present=102|active=102|hourly=102`；另以 Vercel Schedule 手动触发并完成 80 条岗位，证明 Schedule 确实启动正式 crawl-to-inbox workflow。)_
