...
- [ ] 5.7 Implement bounded retry, cooldown, and public re-evaluation for temporary or unconfirmed sources, with `POLICY_DENIED` as a terminal stop until policy changes.
- [ ] 5.8 Add migration, repository, state-transition, deduplication, and 100-source queue tests; verify every fixture receives one outcome without requiring a source-specific parser.
